...
foreach ($requiredManagerFeature in @('gh api', 'MANAGER_ASSET_NAME', 'release_zips/*.apk')) {
    if ($batch -notmatch [regex]::Escape($requiredManagerFeature)) {
        $failures.Add("batch Release is missing manager APK handling: $requiredManagerFeature")
    }
}
if ($batch -notmatch '6\.1\.X') {
    $failures.Add('batch Release title or notes is missing the 6.1.X label')
}
